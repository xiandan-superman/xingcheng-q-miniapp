...
Component({
  ...
  methods:{
    ...
    restart(){this.stop();if(!this._ready)return;const app=getApp&&getApp(),still=this.properties.motion==='still'||!!(app&&app.globalData&&app.globalData.reduceMotion)||!this._visible;if(still){this.setData({phase:'fallback'});return}const choices=['sleep','snack','asteroid'];const pick=choices[Math.floor(Math.random()*choices.length)];this._last=pick;this.playCanvasMode(pick,pick==='sleep'?.18:.65)},
